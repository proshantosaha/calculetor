 ...
 function calcBoxGenerator(clickedKey){
    if(clickedKey == "back" || clickedKey == "C" || clickedKey == "Delete" || clickedKey == "Backspace" ){
        ...
        if(clickedKey == "*" || clickedKey == "+" || clickedKey == "/" || clickedKey == "%" || clickedKey == "-" || clickedKey == "x"){
...
function calculator(nums, oprs){
    let i = 0;
    let op;
    let answer = nums.reduce(function(preValue, currentValue){
        op = oprs[i++];
        console.log("op:", op);

        if(op == "x" || op == "*") return preValue * currentValue;
        else if(op == "+") return preValue + currentValue;
        else if(op == "-") return preValue - currentValue;
        else if(op == "/") return preValue / currentValue;
        else if(op == "%") return preValue % currentValue;
         else return  "none"
